...
- `crates/kasane-gfm`     Leaf crate depending on `kasane-ir` alone: what GFM does to a
  ...
  sees it — leaving two: the empty-id fallback (`EMPTY_FALLBACK`), a
  deliberate choice rather than a construction defect, and a pre-existing,
  still-open one where a heading's empty inline code span renders as a
  space `rendered_text` does not model, so the anchor kasane embeds is a
  dead cross-reference against GitHub's own render.
